...
APPLESCRIPT
    ...
    echo "  4. Windows will install (~20-30 min), then auto-logon runs firstlogin.ps1"
    echo "  5. Verify SSH works: mise wr -- - <<< 'Write-Host ok'"
    echo ""
    echo "Post-install (run from VM desktop via RDP, not SSH):"
    echo "  mise wr -- config/windows/update-windows.ps1   # Windows Update (E_ACCESSDENIED over SSH)"
    echo ""
    echo "Post-install (optional, via SSH -- reboot first so Developer Mode is active):"
    echo "  mise wr -- config/windows/personalize.ps1      # Install tools, upgrade apps"
    echo "  mise wr -- config/windows/debloat.ps1          # Disable ads/suggestions"
    echo ""
