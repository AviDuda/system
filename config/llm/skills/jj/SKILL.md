...
- **`jj push` signs commits with their real dates, then pushes.** It moves the `trunk()` bookmark to the topmost public commit (never a local/private head — unlike a bare `jj bookmark advance`, which can land on one), re-signs every commit being published with its author time as the committer (so GitHub shows each commit's real author date instead of the single push instant), then pushes. `--no-advance` skips the bookmark move and pushes the current position; `--dry-run` reports without changing anything. Use `jj push` rather than `jj bookmark advance && jj git push`. Moving a bookmark backward needs `jj bookmark set <bm> -r <head> --allow-backwards`.
...
- **Auto-fix with `jj reorder`.** When public work sits on top of (or among) local commits so the trunk bookmark can't reach all of it without touching a private commit, `jj reorder` collects ALL public commits onto `trunk()` in order, stacks every local commit above them, puts the working copy back on top, advances the trunk bookmark to the topmost public commit, and `--dry-run`s the push (no remote mutation — it prints the exact `jj git push --bookmark <bm>` to finish). Idempotent: a no-op on an already-correct stack. All revsets are computed up front — recomputing after the first rebase would miss the locals once they're stranded. Scope: one contiguous public run and one contiguous local run; anchor is `trunk()`.
- **Stamp a commit's committer time with `jj stamp`.** jj stamps the COMMITTER time "now" on every rewrite (author time is preserved), and jj/GitHub display the committer time. `jj stamp <rev> [<TIME>]` resets the committer time — with no TIME, to the rev's own author time; with an RFC 3339 TIME (e.g. `2026-08-11T09:00:00+02:00`), to that explicit time. Preserves content, parents, description, and author. `jj push` signs each published commit with its author time via the same `JJ_TIMESTAMP` mechanism.
- **A pushed commit's GitHub-visible date is its AUTHOR time, which is set when the change is created and preserved through edits, `jj commit`, `jj describe`, rebases, and `jj push`.** A change that accumulated over a long stretch shows its creation time ("when the work started"). To change it, use `jj metaedit --author-timestamp '<RFC 3339>'` (set a specific time, e.g. backdating) or `--update-author-timestamp` (set to now). The author timestamp is not settable via `jj describe` in 0.43. Caveat: a *backdated* author (set far in the past) does not keep its date on push — sign-on-push resets the committer (the GitHub-displayed date) to the push instant, so only normal author times survive `jj push`.
...
- The user config defines `jj sync` (fetch + rebase your work onto the repo's `trunk()`) and `jj push` (advance trunk to the topmost public commit, re-sign+stamp the pushed commits so their GitHub-visible date equals their author time, then push). Use those for the daily loop; the raw commands below are what they do. (`jj push` assumes direct-to-trunk; PR-based repos need a feature bookmark set up first.)
