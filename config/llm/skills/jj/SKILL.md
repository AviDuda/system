...
- **`jj c <paths>... -m "<msg>"`** — git-style commit (describe + create a fresh change on top); for a previously-empty change it also resets the author date to now, so the commit shows when it was committed. Prefer `jj c` over bare `jj commit` when committing finished work.
- `jj commit <paths>... -m "<msg>"` — like `jj c` but leaves the author date as-is. (Agent default: use `jj c`.)
...
- **`jj push` moves the `trunk()` bookmark to the topmost PUBLIC commit (never a local/private head), then pushes** — private/local commits (`LOCAL:`/`wip:`/`private:`) stay unpushed. Signing is via `git.sign-on-push`; GitHub shows the normal git committer (push) date (see below). `--no-advance` skips the move; `--dry-run` reports only. Moving a bookmark backward needs `jj bookmark set <bm> -r <head> --allow-backwards`.
...
- **`jj stamp <rev> [<TIME>]`** manually sets a commit's COMMITTER time (no TIME = the rev's author time; an RFC 3339 TIME = explicit). A manual override — `jj push` doesn't do this.
- **GitHub shows the COMMITTER date, not the author date.** jj bumps the committer to now on every rewrite and signs on push, so a pushed batch shows the push time — git-normal. The AUTHOR date is set when a change is created (often an auto-created empty change) and preserved through rewrites — why a change committed "now" can carry an older-looking author date. To align the author date with when you committed, use `jj c` (resets it for a previously-empty change) or `jj metaedit --update-author-timestamp` (now) / `--author-timestamp '<RFC 3339>'` (specific).
